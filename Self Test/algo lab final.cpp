...
int main()
{
    ...
    while (cur_weight > 0)
    {
        ...
        if (cur_weight >= 0)
            printf("Added object %d (%d Tk., %dKg) completely in the bag. Space left: %d.\n", item + 1, value[item], weight[item], cur_weight);
        else
        {
            int item_percent = (int) ((1 + (float) cur_weight / weight[item]) * 100);
            printf("Added %d%% (%d Tk., %dKg) of object %d in the bag.\n", item_percent, value[item], weight[item], item + 1);
            total_profit -= value[item];
            total_profit += (1 + (float)cur_weight / weight[item]) * value[item];
        }
    }

    printf("\nFilled the bag with objects worth %.2f Tk.\n", total_profit);
}
